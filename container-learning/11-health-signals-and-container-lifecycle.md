...
3. Stop the app from responding correctly somehow (for a safe experiment: `docker exec notes-health kill 1` sends SIGTERM to the main process, which should - if Lesson 14's shutdown handling works - cause the container to exit rather than hang). Watch what `docker ps` reports as the container goes down.
