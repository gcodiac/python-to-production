# Lesson 25 — Building a Manual Release Quality Gate
...
### Why the gate tests both databases

`build` runs **once**, and both smoke tests run against that single image:

```text
docker build  ->  notes-app:local  -+->  container-smoke-test.sh   (SQLite)
                                    |
                                    +->  postgres-smoke-test.sh    (PostgreSQL)
```

That structure is the point. If the gate had to build a different image per database, the application would not be portable - it would just have two builds, and "the artefact we tested" would no longer be "the artefact we ship". One build, two backends, one release candidate.

This also catches a specific and easily-missed class of bug: SQL or schema behaviour that happens to work on SQLite and fails on PostgreSQL. Running the application's own test suite proves the Python is fine; running the *built image* against a real PostgreSQL server proves the deployable artefact is.
...
2. `container-test`, `postgres-test`, `scan`, and `sbom` all depend on `build` having already produced a real image. What would happen if you ran `make check` on a machine without Docker installed at all? Which specific targets would fail, and which would still succeed?
3. `postgres-smoke-test.sh` queries the database with `psql` after creating a note over HTTP, instead of trusting the application's own `/ready` response. Why is that a materially stronger check?
4. Why does this lesson - and this whole track - insist on you running every one of these commands by hand at least once, *before* relying on `make check`?

## Practical exercise

Run the complete gate yourself, top to bottom, exactly as listed above (or via `make check` if you have `make` installed). For any step that isn't a clean PASS, apply Lesson 17's triage workflow and record your decision, the same way `SECURITY.md` already does for this project's current (clean) results.
