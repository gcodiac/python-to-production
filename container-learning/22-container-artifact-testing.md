# Lesson 22 — Container Artifact Testing
...
1. The script's very first real check is "no secret appears in the image's own config," using `docker inspect`, not a simple string grep through the filesystem. Given Lesson 20's "deleted secrets still exist in old layers" lesson, why is checking `docker inspect`'s reported `Env` the *right* check here specifically, for *this* project (which never bakes a secret in at all, rather than one that bakes one in and then tries to remove it)?
