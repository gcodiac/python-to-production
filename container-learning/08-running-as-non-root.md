...
1. The Dockerfile creates a group and user both explicitly at uid/gid `1000`, then later does `USER 1000:1000` (numeric), not `USER appuser` (by name). Lesson 14 covers exactly why Hadolint prefers the numeric form - what's the risk with the named form that the numeric form avoids?
