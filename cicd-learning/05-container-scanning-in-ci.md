# Lesson 05 — Container Scanning in CI
...
* **`ignore-unfixed: true`** — don't fail on a vulnerability with no available fix yet. There is nothing actionable you can do about it today except accept and document it (Lesson 09's spirit, applied here) - failing the build over it just trains people to ignore red X's.
...
3. Why does scanning happen *after* the build but *before* anything gets published (in `release.yml`)? What would go wrong about the "build once, promote many" story (Lesson 07) if scanning happened after publishing instead?
