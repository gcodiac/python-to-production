# Lesson 15 — Ready for Cloud Deployment
...
* **No GitHub Environments or required approvals configured** - documented as a recommendation (Lesson 13), not applied.
* **No branch protection configured** - documented as a recommendation (Lesson 14), not applied.
...
Notice the OIDC pattern from Lesson 11 doesn't get reinvented for cloud access - it gets *extended*. The same "short-lived, workflow-scoped identity instead of a long-lived stored key" principle that signed this project's image will be how Stage 4's workflows authenticate to real cloud infrastructure.
...
2. Why does it make sense that the *same* OIDC concept from Lesson 11 (GitHub identity → short-lived credential) is what Stage 4 will reach for, rather than a completely different cloud-authentication approach?
